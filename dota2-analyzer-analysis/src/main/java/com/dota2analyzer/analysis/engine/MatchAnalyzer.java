package com.dota2analyzer.analysis.engine;
...
public class MatchAnalyzer {
    ...
    private final DotaDataProvider client;
    ...
    public MatchAnalyzer(DotaDataProvider client, HeroDataCache heroData, MatchCache cache,
    ...
    public List<MatchAnalysisResult> analyzeRecent(long accountId, int desiredCount, int fetchLimit,
        ...
        if (cacheOnly) {
            ...
            if (matches.isEmpty()) {
                log.info("Recent matches cache empty. Fetching from data service AccountId={} Target={}", accountId, fetchLimit);
                ...
                if (shouldRefresh) {
                    log.info("Recent matches cache outdated. Fetching from data service AccountId={} Target={}", accountId, fetchLimit);
                    matches = fetchRecentMatchesWithPaging(accountId, fetchLimit);
                    if (!matches.isEmpty()) {
                        cache.saveRecentMatches(accountId, matches);
                    }
    ...
    private boolean shouldRefreshRecentMatches(long accountId, List<RecentMatch> cached) {
        try {
            List<RecentMatch> latest = client.getPlayerMatches(accountId, 1, 0, 7);
            if (latest.isEmpty()) {
                log.warn("Data service returned no recent matches for {}", accountId);
                return false;
            }
    ...
    public MatchAnalysisResult analyzeMatch(RecentMatch match, long accountId, boolean requestParse, boolean onlyPos1) {
        ...
        if (detail == null) {
            if (cacheOnly) {
                log.warn("Match cache miss and cache-only enabled. Skipping external fetch.");
                return null;
            } else {
                if (requestParse) {
                    client.requestParse(match.getMatchId());
                }
                log.info("Match cache miss for {}, fetching from data service", match.getMatchId());
    ...
    private MistakesResult detectMistakes(RecentMatch match, PlayerDetail player, List<String> enemyHeroes,
        ...
        if (shouldFlagLowPushContribution(match, player, isSupport, teamTowerDamage)) {
            mistakes.add("\u63A8\u8FDB\u8D21\u732E\u4E0D\u8DB3");
            suggestions.add("\u4E2D\u540E\u671F\u4E3B\u52A8\u63A8\u52A8\u7EBF\u6743\u548C\u5854\u538B\u529B\uFF0C\u6269\u5927\u4F18\u52BF\u6216\u6263\u56DE\u8282\u594F");
        }
